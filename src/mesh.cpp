...
#define COST_INTERSECT 1.0
#define COST_SPLIT 0.3

double Mesh::sah(const BBox& bbox, Axis axis, double splitPos, const vector<int>& tList)
{
    BBox bbLeft, bbRight;
...
void Mesh::build(KDTreeNode& node, const BBox& bbox, const vector<int>& tList, int depth)
{
	if (tList.size() < MAX_TRIANGLES_PER_LEAF || depth > MAX_TREE_DEPTH) {
		...
		if(useSAH)
        {
            ...
            {
                double current = sah(bbox, axis, p, tList);
                if(current < min_sah)
                {
                    splitPos = p;
                    min_sah = current;
